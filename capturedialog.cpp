...
QPoint CCaptureDialog::Position(QPoint(0,0));


CCaptureDialog::CCaptureDialog(QWidget *parent) :
...
CCaptureDialog::~CCaptureDialog()
{

}

 void CCaptureDialog::setGroupBoxSize(QSize size)
 {
     ...
     groupBox->setStyleSheet("QGroupBox{ border-top-left-radius:0px;  border-top-right-radius:0px;  background-color:rgb(30,40,150)}");
     label = new CInCaptureLabel(groupBox);
     ...
     bottomWidget = new CInCaptureWidget(this);
     bottomWidget->setGeometry(1,normalRect.height()-75, normalRect.width()-2, 74);
     //bottomWidget->setStyleSheet("QWidget{background-color:rgb(229,229,229)}");
     ...
     connect(browseButton,&QPushButton::clicked, this , &CCaptureDialog::slotBrowseButtonClicked);
     ...
     connect(saveButton, &QPushButton::clicked, this, &CCaptureDialog::slotSaveBottonClicked);
     ...
     connect(saveToClipBoard, &QPushButton::clicked, this, &CCaptureDialog::slotSaveToClipBoard);
     setButton();


 }

 void CCaptureDialog::slotBrowseButtonClicked()
 {
     CFileSystemDialog * dialog = new CFileSystemDialog;
     connect(dialog, &CFileSystemDialog::signalPathSelected, [this](QString filePath){ lineEdit->setText(filePath); });
     dialog->exec();
 }


 void CCaptureDialog::slotBottomWidgetMouseLeftDown()
 {
         Position=this->pos();
 }

 //保存至剪切板
void CCaptureDialog::slotSaveToClipBoard()
{
...
//使用videoWidget0移动窗体
 void CCaptureDialog::slotSignalMovePoint(QPoint point)
 {
         this->move(point+Position);
 }

void CCaptureDialog::region(const QPoint &cursorGlobalPoint)
{
...
void CCaptureDialog::mouseReleaseEvent(QMouseEvent *event)
{
...
void CCaptureDialog::mousePressEvent(QMouseEvent *event)
{
    switch(event->button()) {
    case Qt::LeftButton:
...
void CCaptureDialog::setPixmap(QPixmap pixmap)
{
    this->pixmap = pixmap;
...
void CCaptureDialog::mouseMoveEvent(QMouseEvent *event)
{
...
void CCaptureDialog::setButton()
{
    ...
    closeButton->setGeometry(a-40, 10, 28, 20);     //将关闭按钮放在最右上角
    closeButton->setCursor(QCursor(Qt::ArrowCursor));
    minimumButton->setGeometry(a-74,10, 28, 20);
    ...
    closeButton->setStyleSheet("QPushButton{border-radius:8px; background-color:rgb(0,91,172);}"
                                                  "QPushButton::hover{background-color:rgb(90,190,81);}");
    minimumButton->setStyleSheet("QPushButton{border-radius:8px; background-color:rgb(0,91,172);}"
                                                         "QPushButton::hover{ background-color:rgb(90,190,81);}");
...
void CCaptureDialog::slotSaveBottonClicked()
{
